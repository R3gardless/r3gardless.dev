...
- [ ] `KNOWLEDGE_BASE_PATH`가 올바른 KNOWLEDGE_BASE 루트를 가리킨다.
...
- [ ] Secret `KNOWLEDGE_BASE_TOKEN`이 private KNOWLEDGE_BASE read 권한을 가진다.
...
- [ ] KNOWLEDGE_BASE 내부 `.md` 링크가 내부 블로그 링크, `source_url`, 또는 텍스트 강등으로 처리된다.

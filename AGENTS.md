...
Next.js App Router 기반의 정적 블로그입니다. 콘텐츠 원본은 개인 KB의 Markdown이고, 빌드 시 `publish: true` 문서만 `content/posts/`와 `public/content/`로 export한 뒤 렌더링합니다.

## 스택

- Framework: Next.js 16 App Router, React 19, TypeScript
- Styling: Tailwind CSS v4, CSS variables, `src/styles/globals.css`
- Content: KB Markdown -> `content/posts/` -> static render
...
├── .codex/skills/          # repo-local Codex skill
├── content/posts/          # KB에서 export된 publish:true Markdown. generated, gitignore.
...
├── scripts/                # sync-kb, build-content, build-post-meta, check-links, smoke-build-output
├── src/
│   ├── app/                # Next.js routes
│   ├── components/         # UI, sections, templates, layout
│   ├── libs/content/       # KB scanner, exporter, resolver, Markdown renderer
│   ├── styles/             # globals, Markdown body, code theme, masonry
│   ├── types/
│   └── utils/
├── src/__tests__/          # src 구조를 미러링하는 단위 테스트
├── tests/fixtures/kb/      # 하네스용 fixture KB
...
KB 원본은 읽기 전용입니다. 이 레포의 파이프라인은 generated output만 쓰며, KB 파일을 수정하지 않습니다. 예외는 마이그레이션 회고 노트 1건 추가가 명시된 작업뿐입니다.

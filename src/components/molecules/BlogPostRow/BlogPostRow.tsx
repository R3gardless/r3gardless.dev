...
import { Heading, DateText } from '@/components/atoms/Typography';
...
export const BlogPostRow = ({
  ...
  return (
    ...
    <div className={`${baseStyles} ${className}`} onClick={handlePostClick} data-theme={theme}>
      {/* 왼쪽 콘텐츠 영역 */}
      <div className="flex-1 space-y-3 md:space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((tag, index) => (
            ...
            >
              <Tag text={tag} theme={theme} className="transition-opacity hover:opacity-80" />
            </div>
          ))}
        </div>
      </div>

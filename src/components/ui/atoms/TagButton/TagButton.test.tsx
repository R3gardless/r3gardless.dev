...
describe('TagButton', () => {
  ...
  describe('클릭 기능', () => {
    it('onClick이 없을 때 span 엘리먼트로 렌더링된다', () => {
      render(<TagButton text="non-clickable" />);
    ...
    it('onClick이 있을 때 button 엘리먼트로 렌더링된다', () => {
      ...
      render(<TagButton text="clickable" onClick={handleClick} />);
    ...
    it('여러 번 클릭해도 이벤트가 올바르게 호출된다', () => {
      const handleClick = vi.fn();
      render(<TagButton text="multi-click" onClick={handleClick} />);
  ...
  describe('엣지 케이스', () => {
    it('공백만 있는 텍스트도 # 접두사가 추가된다', () => {
      const { container } = render(<TagButton text="   " />);
    ...
    it('여러 개의 # 문자로 시작하는 텍스트는 중복 추가되지 않는다', () => {
      render(<TagButton text="##double-hash" />);
  ...
  describe('접근성', () => {
    it('클릭 가능한 태그는 button role을 가진다', () => {
      const handleClick = vi.fn();
      render(<TagButton text="accessible" onClick={handleClick} />);
      const tag = screen.getByRole('button');
      expect(tag).toHaveTextContent('#accessible');
    });

    it('클릭 불가능한 태그는 button role을 가지지 않는다', () => {
      render(<TagButton text="non-accessible" />);
  ...
  describe('스타일 조합', () => {
    ...
    it('className이 undefined일 때도 올바르게 렌더링된다', () => {
      render(<TagButton text="no-class" className={undefined} />);
      const tag = screen.getByText('#no-class');
      expect(tag).toBeInTheDocument();
    });

    it('빈 className일 때도 올바르게 렌더링된다', () => {
      render(<TagButton text="empty-class" className="" />);
  ...
  describe('컴포넌트 선택 로직', () => {
    it('onClick이 정의되지 않으면 Component는 span이다', () => {
      render(<TagButton text="test" />);
      const tag = screen.getByText('#test');
      expect(tag.tagName).toBe('SPAN');
    });

    it('onClick이 null이면 Component는 span이다', () => {
      render(<TagButton text="test" onClick={null as unknown as () => void} />);
      const tag = screen.getByText('#test');
      expect(tag.tagName).toBe('SPAN');
    });

    it('onClick이 undefined이면 Component는 span이다', () => {
      render(<TagButton text="test" onClick={undefined} />);
      const tag = screen.getByText('#test');
      expect(tag.tagName).toBe('SPAN');
    });
    ...
    it('onClick이 함수이면 Component는 button이다', () => {
      const handleClick = vi.fn();
      render(<TagButton text="test" onClick={handleClick} />);
  ...
  describe('텍스트 처리 로직', () => {
    it('text.startsWith("#")가 true일 때 # 접두사가 추가되지 않는다', () => {
      render(<TagButton text="#already-has-hash" />);
  ...
  describe('삼항 연산자 분기 테스트', () => {
    it('onClick이 truthy할 때 button으로 렌더링', () => {
      const mockFn = vi.fn();
      render(<TagButton text="test" onClick={mockFn} />);
      const tag = getParentElement(screen.getByText('#test'));
      expect(tag.tagName).toBe('BUTTON');
    });
  ...
  describe('CSS 클래스 조합', () => {
    it('baseStyles, themeStyles, className이 모두 조합된다', () => {
      render(<TagButton text="test" className="custom-class" />);
      const tag = getParentElement(screen.getByText('#test'));
    ...
    it('className이 빈 문자열일 때도 조합된다', () => {
      render(<TagButton text="test" className="" />);
      const tag = getParentElement(screen.getByText('#test'));
      expect(tag).toHaveClass(
        'inline-flex',
        'items-center',
  ...
  describe('추가 브랜치 커버리지 테스트', () => {
    it('다양한 falsy 값들로 onClick을 테스트', () => {
      // onClick이 false인 경우
      render(<TagButton text="test-false" onClick={false as unknown as () => void} />);
      ...
      // onClick이 0인 경우
      render(<TagButton text="test-zero" onClick={0 as unknown as () => void} />);
      ...
      // onClick이 빈 문자열인 경우
      render(<TagButton text="test-empty" onClick={'' as unknown as () => void} />);
    ...
    it('다양한 truthy 값들로 onClick을 테스트', () => {
      ...
      // onClick이 일반 함수인 경우
      render(<TagButton text="test-fn" onClick={mockFn} />);
      ...
      // onClick이 화살표 함수인 경우
      render(<TagButton text="test-arrow" onClick={() => {}} />);
    ...
    it('text 처리의 모든 브랜치를 테스트', () => {
      ...
      // 특수한 경우: '#'이 중간에 있는 경우
      render(<TagButton text="has#middle" />);
    ...
    it('className 처리의 모든 브랜치를 테스트', () => {
      // className이 정의된 경우
      render(<TagButton text="with-class" className="test-class" />);
      ...
      render(<TagButton text="multi-class" className="class1 class2 class3" />);
    ...
    it('이벤트 핸들링의 모든 브랜치를 테스트', () => {
      ...
      // 클릭 가능한 태그
      render(<TagButton text="clickable" onClick={mockFn} />);
  ...
  describe('isClicked 상태 테스트', () => {
    it('isClicked가 false일 때 기본 스타일이 적용된다', () => {
      render(<TagButton text="normal" isClicked={false} />);
    ...
    it('isClicked가 true일 때 클릭된 상태 스타일이 적용된다', () => {
      render(<TagButton text="clicked" isClicked={true} />);
    ...
    it('isClicked가 undefined일 때 기본값 false로 동작한다', () => {
      render(<TagButton text="default" isClicked={undefined} />);
  ...
  describe('onRemove 기능 테스트', () => {
    it('isClicked가 false이고 onRemove가 있어도 X 아이콘이 표시되지 않는다', () => {
      ...
      render(<TagButton text="no-x" isClicked={false} onRemove={handleRemove} />);

      expect(screen.queryByLabelText('태그 제거')).not.toBeInTheDocument();
    });

    it('isClicked가 true이고 onRemove가 없으면 X 아이콘이 표시되지 않는다', () => {
      render(<TagButton text="no-remove" isClicked={true} />);
    ...
    it('isClicked가 true이고 onRemove가 있으면 X 아이콘이 표시된다', () => {
      const handleRemove = vi.fn();
      render(<TagButton text="with-x" isClicked={true} onRemove={handleRemove} />);
    ...
    it('X 아이콘 클릭 시 onRemove가 호출된다', () => {
      const handleRemove = vi.fn();
      render(<TagButton text="removable" isClicked={true} onRemove={handleRemove} />);
    ...
    it('X 아이콘에 올바른 스타일이 적용된다', () => {
      const handleRemove = vi.fn();
      render(<TagButton text="styled-x" isClicked={true} onRemove={handleRemove} />);
  ...
  describe('호버 스타일 테스트', () => {
    it('onClick이 있을 때 호버 스타일이 적용된다', () => {
      const handleClick = vi.fn();
      render(<TagButton text="hoverable" onClick={handleClick} />);
    ...
    it('onClick이 없을 때 호버 스타일이 적용되지 않는다', () => {
      render(<TagButton text="non-hoverable" />);

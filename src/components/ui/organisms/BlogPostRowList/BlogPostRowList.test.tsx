...
describe('BlogPostRowList', () => {
  it('renders correctly with posts', () => {
    render(<BlogPostRowList posts={mockPosts} />);
  ...
  it('renders loading state', () => {
    const { container } = render(<BlogPostRowList posts={[]} isLoading={true} />);
  ...
  it('renders empty state', () => {
    render(<BlogPostRowList posts={[]} isLoading={false} emptyMessage="No posts available" />);
  ...
  it('handles category click', () => {
    ...
    render(<BlogPostRowList posts={mockPosts} onCategoryClick={handleCategoryClick} />);
  ...
  it('handles tag click', () => {
    ...
    render(<BlogPostRowList posts={mockPosts} onTagClick={handleTagClick} />);
  ...
  it('does not render pagination when totalPages is 1', () => {
    render(
      <BlogPostRowList posts={mockPosts} currentPage={1} totalPages={1} onPageChange={vi.fn()} />,
    );
  ...
  it('does not render pagination when onPageChange is not provided', () => {
    render(<BlogPostRowList posts={mockPosts} currentPage={1} totalPages={5} />);

    expect(screen.queryByRole('navigation', { name: '페이지네이션' })).not.toBeInTheDocument();
  });

  it('applies custom className', () => {
    const { container } = render(<BlogPostRowList posts={mockPosts} className="custom-class" />);
  ...
  it('forwards ref correctly', () => {
    ...
    render(<BlogPostRowList ref={ref} posts={mockPosts} />);

    expect(ref).toHaveBeenCalled();
  });

  it('renders sort controls when showSort is true', () => {
    render(<BlogPostRowList posts={mockPosts} showSort={true} sortDirection="desc" />);
  ...
  it('does not render sort controls when showSort is false', () => {
    render(<BlogPostRowList posts={mockPosts} showSort={false} />);
  ...
  it('disables currently selected sort direction', () => {
    render(<BlogPostRowList posts={mockPosts} showSort={true} sortDirection="asc" />);

...
import { BlogListSidebar } from '.';

// 테스트용 샘플 데이터
const SAMPLE_CATEGORIES = ['전체', 'React', 'JavaScript'];
const SAMPLE_TAGS = ['React', 'TypeScript', 'Next.js', 'Node.js'];

describe('BlogListSidebar', () => {
  describe('기본 렌더링', () => {
    it('BlogListSidebar가 렌더링된다', () => {
      render(<BlogListSidebar categories={SAMPLE_CATEGORIES} tags={SAMPLE_TAGS} />);
    ...
    it('isHidden=true인 경우 사이드바가 렌더링되지 않는다', () => {
      const { container } = render(
        <BlogListSidebar categories={SAMPLE_CATEGORIES} tags={SAMPLE_TAGS} isHidden={true} />,
      );
  ...
  describe('조건부 렌더링', () => {
    ...
    it('showMoreTags=false인 경우 태그 더보기가 표시되지 않는다', () => {
      render(
        <BlogListSidebar categories={SAMPLE_CATEGORIES} tags={SAMPLE_TAGS} showMoreTags={false} />,
      );
